...
/**
 * Multibrot fractal algorithm/strategy class.
 * @author Antonio Spoleto Junior
 */
public class MultibrotStrategy extends FractalStrategy
{
    private double fractionalIterations[][];
    private int order;
    private Double max;

    public MultibrotStrategy(ColorPalette palette, int order)
    {
        super(palette);
        this.order = order;
    }
    ...
    /**
     * Iterate pixel value with Mandelbrot algorithm with choosen exponent
     * The function being iterated is the following: f(z) = z^exp+c
    ...
    private double iterate(Complex c, int iterations, boolean smoothing)
    {
        ...
        for (int i = 0; i < iterations; i++)//Dwell limit
        {
            ...
            if (modulus > 2)
            {
                double convergenceDegree = (i+1 - (Math.log(Math.log(modulus))) / Math.log(order));
                synchronized (max)
                {
